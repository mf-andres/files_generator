...
def test_returns_file_with_proper_complex_file_name():
    class_file_contents = class_file_contents_mother.get_complex_class_file_contents()
    file_generator = ItemFileGenerator()
    file = file_generator.generate(class_file_contents)
    file_has_proper_file_name = file.file_name == "complex_example_item.py"
    assert file_has_proper_file_name


def test_returns_file_with_proper_complex_file_contents():
    class_file_contents = class_file_contents_mother.get_complex_class_file_contents()
    file_generator = ItemFileGenerator()
    file = file_generator.generate(class_file_contents)
    expected_file_file_contents = """class ComplexExampleItem(BaseModel):
    int_: int
    float_: float
    str_: str
    list_: List[int]
